...
from doa_zero_eeg.models import mlp_regressor, lstm_regressor
...
signals_to_scale = ['BIS', 'CO₂fe', 'FC', 'PNIm', 'SpO₂'] # scaling all explanatory variables and target
scaler = StandardScaler() # also try MinMax scaling
...
mlp_model = mlp_regressor.MLPBisPredictor(
    input_dim=n_features, 
    hidden_dim=32, 
    output_dim=1, 
    dropout=0.3,
).to(device)
optimizer = torch.optim.Adam(mlp_model.parameters(), lr=0.0001, weight_decay=1e-5)
criterion = torch.nn.L1Loss() # MAE error


# Training loop
logger.info(f"Start of the MLP regressor training on device: {device}")
num_epochs = 10
train_losses = list()
val_losses = list()
...
plt.grid(True)
plt.tight_layout()
plt.savefig(dir.joinpath(Path(f"mlp_training_validation_loss_{num_epochs}_epochs.png")))
plt.close()

# TODO: compute correct MAE on test set (at this point all the test set in put into memory...)
# mlp_model.eval()  
# with torch.no_grad(): 
#     test_predictions = mlp_model(test_data)
